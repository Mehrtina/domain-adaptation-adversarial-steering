...
OUTPUT_METRICS = str(RESULTS_DIR / "PGD_bootstrap_CI_metrics.csv")
OUTPUT_TESTS = str(RESULTS_DIR / "PGD_wilcoxon_significance_tests.csv")
...
def parse_filename(filename):
    ...
    return architecture, strategy, epsilon, alpha
...
def compute_metrics_for_file(csv_path):
    ...
    architecture, strategy, epsilon, alpha = parse_filename(filename)
...
def run_significance_tests(csv_files):
    ...
    # Compare only key pairs at epsilon = 0.03
    key_epsilon = 0.03
...
if __name__ == "__main__":
    ...
    for folder in INPUT_FOLDERS:
        all_files.extend(glob.glob(os.path.join(folder, "**", "*.csv"), recursive=True))
