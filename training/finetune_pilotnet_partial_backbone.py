...
Freeze strategy: partial — only first 3 layers frozen, remaining layers trainable.
Produces: WEIGHTS_PILOTNET_FT_US in config.py
"""
import sys
...
from config import FINETUNE_DATA_DIR, LOGS_DIR, WEIGHTS_PILOTNET_FLIPPED, WEIGHTS_PILOTNET_PARTIAL_FT_FLIPPED, BATCH_SIZE, NUM_EPOCHS_FT, LEARNING_RATE

FINETUNE_DATA_DIR = Path(FINETUNE_DATA_DIR)
LOGS_DIR = Path(LOGS_DIR)

MODEL_CHECKPOINT_IN  = Path(WEIGHTS_PILOTNET_FLIPPED)
LOG_DIR              = LOGS_DIR    / "logs_partial_backbone_finetuned_flipped"
MODEL_CHECKPOINT_OUT = Path(WEIGHTS_PILOTNET_PARTIAL_FT_FLIPPED)
...
def finetune():
    ...
    # Partial freeze: only first 3 layers frozen, remaining layers stay trainable
