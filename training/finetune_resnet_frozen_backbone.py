...
from config import (
    ...
    WEIGHTS_RESNET_US, WEIGHTS_RESNET_FT_HEAD_ONLY_US, BATCH_SIZE, NUM_EPOCHS_FT, LEARNING_RATE
)
...
LOG_DIR              = LOGS_DIR / "logs_frozen_backbone_head_only_finetuned_ResNet"
MODEL_CHECKPOINT_OUT = Path(WEIGHTS_RESNET_FT_HEAD_ONLY_US)
...
def freeze_backbone_train_head_only(model):
    """Frozen-backbone / head-only fine-tuning: freeze all layers except the final output layer."""
...
def finetune():
    ...
    model = freeze_backbone_train_head_only(model)
