...
from config import RESULTS_FGSM_DIR, RESULTS_DIR, FIGURES_DIR


INPUT_FOLDER = str(RESULTS_FGSM_DIR)
OUTPUT_CSV = str(RESULTS_DIR / "FGSM_error_distribution_long_format.csv")
OUTPUT_FOLDER = str(FIGURES_DIR / "fgsm_error_distribution_plots")
...
for csv_path in glob.glob(os.path.join(INPUT_FOLDER, "**", "*.csv"), recursive=True):
...
for architecture in sorted(df_all["Architecture"].unique()):
    for epsilon in sorted(df_all["Epsilon"].dropna().unique()):
        ...
        plt.ylabel("Absolute steering error under FGSM (degrees)")
        plt.xlabel("Training strategy")
        plt.title(f"{architecture}, FGSM ε={epsilon}")
        plt.xticks(rotation=25, ha="right")
        plt.tight_layout()

        out_path = os.path.join(
            OUTPUT_FOLDER,
            f"{architecture}_FGSM_eps{epsilon}_absolute_error_boxplot.png"
        )
